...
Full implementation detail for `delivery_layer/` and `examples/policy-reader/`: repo layout, setup, the Rime integration table, ingestion internals, the navigator, conversation routing, the companion narrator, the web demo, timestamp fidelity, design decisions, known limitations, and failure behaviour.

Start with [`README.md`](../README.md) for the problem and the claim, and [`RIME_EVIDENCE.md`](../RIME_EVIDENCE.md) for the acceptance test and results. This file is everything else.
